...
window_size = (VID_CAP.get(cv.CAP_PROP_FRAME_WIDTH),
               VID_CAP.get(cv.CAP_PROP_FRAME_HEIGHT))
screen = pygame.display.set_mode(window_size)

# Bird and pipe initialize
bird_img = pygame.image.load('./img/bird_sprite.png')
bird_img = pygame.transform.scale(
    bird_img, (bird_img.get_width() / 10, bird_img.get_height() / 10))
bird_frame = bird_img.get_rect()
bird_frame.center = (window_size[0] // 10, window_size[1] // 6)

pipe_frames = deque()
pipe_img = pygame.image.load('./img/pipe_sprite_single.png')
pipe_starting_template = pipe_img.get_rect()
space_between_pipes = 170
...
with mp_face_mesh.FaceMesh(max_num_faces=1, refine_landmarks=True, min_detection_confidence=0.5, min_tracking_confidence=0.5) as face_mesh:
    while True:
        ...
        for pf in pipe_frames:
            pf[0].x -= pipe_velocity()  # bottom pipe
            pf[1].x -= pipe_velocity()  # top pipe
        ...
        if pipeSpawnTimer == 0:
            ...
            top.x, top.y = window_size[0], random.randint(
                100 - 1000, window_size[1] - 100 - space_between_pipes - 1000)
